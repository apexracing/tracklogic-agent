...
这个仓库是通用 Harness，不是某个数据领域的分析产品。后续应用可以依赖它构建 Agent，并通过 Tool 或 MCP 读取实时数据，但依赖方向只能从应用指向库：

| 公共 Harness 负责 | 后续应用负责 |
|---|---|
| Agent 有界循环、流式消费和取消 | 领域任务如何拆解、何时触发分析 |
| Model、Tool、Memory 的通用接口 | 数据源字段、查询参数和业务校验规则 |
| Registry、权限检查和输入输出治理 | 领域 Tool/MCP Server 及其鉴权 |
| Team、Workflow 和并发安全 State | 分析 Prompt、计算逻辑、结论与报告格式 |
| RunID、结构化错误和基础运行日志 | 数据保存、聚合、展示和产品交互 |

为什么不能把应用概念提前放进库？因为公共库一旦认识具体数据结构、报告对象或产品流程，其他应用就必须携带无关依赖，`types` 也会逐渐变成领域杂物包。正确扩展方式是：应用实现 `model.Model`、`tool.Tool`、`memory.Memory` 或 `workflow.Node` 等公开接口；只有多个无关应用都需要、且不带领域语义的能力，才考虑下沉到本库。
...
当前 `Workflow` 不是通用 DAG 引擎。它按 `AddNode` 顺序执行顶层节点，并通过 `ConditionNode`、`LoopNode`、`ParallelNode` 组合出分支、循环和局部并发。这种结构更像可组合的控制流解释器：实现简单、行为容易预测，但没有任意边、拓扑排序、持久化恢复或环检测。

选择原则：

- 只需固定多人协作：使用 Team；
- 需要显式步骤、分支、循环或共享 State：使用 Workflow；
- 需要任意 DAG、断点恢复和分布式调度：在当前 Workflow 之上扩展，不能假设已有这些能力。
...
当前代码已有 HTTP Client 超时、`context` 取消、有界循环、结构化错误和 slog 日志。指数退避、断路器和自动降级仍是扩展设计。

不要在所有层同时重试。否则一次用户请求可能形成“网关重试 × Agent 重试 × Provider 重试”的乘法放大。通常应由最了解幂等性和错误分类的层负责：

- HTTP 状态、超时与响应解析：Model Provider 负责分类；应用根据幂等性和预算决定是否重试；
...
| BufferMemory | 已实现 | Agent 内共享，不提供多用户会话隔离或持久化 |
...
| 自动重试/断路器 | 扩展设计 | 第 11 章给出设计方法，当前运行时未接入 |
| Summary/持久化 Memory | 扩展设计 | 当前 Harness 始终创建 BufferMemory |
